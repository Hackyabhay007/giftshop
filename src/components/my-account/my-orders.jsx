...
const MyOrders = ({ orderData }) => {
  ...
  return (
    <div className="profile__ticket" style={{ maxWidth: "800px", margin: "4px auto", padding: "30px", border: "1px solid #ccc", borderRadius: "4px", boxShadow: "0 4px 20px rgba(0, 0, 0, 0.1)", backgroundColor: "#f9f9f9" }}>
      {order_items?.data?.length === 0 ? (
        ...
        <table className="table" style={{ width: "100%", tableLayout: "fixed", fontSize: "16px" }}>
          <thead>
            <tr>
            ...
            {order_items?.data?.map((item) => (
              <tr key={item.id}>
                <td colSpan="4" style={{ padding: "10px", borderBottom: "1px solid #ccc" }}>
                  <div style={{ fontWeight: "bold", fontSize: "18px" }}>#{item.order_id}</div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: "5px" }}>
                    <span style={{ fontSize: "14px" }}>{dayjs(item.created_at).format("MMMM D, YYYY")}</span>
                    <span className={`status ${item.status}`} style={{ fontSize: "14px" }}>
