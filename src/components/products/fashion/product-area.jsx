...
const ProductArea = () => {
  ...
  const { data: products, isError, isLoading } =
    useGetProductTypeQuery({ type: 'HAPPYCARDS' });
  // handleActiveTab
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (activeTab === 'HAPPYCARDS') {
      product_items = products.data
    }
    else if (activeTab === 'RETURNGIFT	') {
      product_items = products.data.filter(p => p.category.name === 'RETURNGIFT	')
    } else if (activeTab === 'CUDDLYCLASS') {
      product_items = products.data.filter(p => p.category.name === 'CUDDLYCLASS')
    } else if (activeTab === 'SUPERNATURALCLASS') {
      product_items = products.data.filter(p => p.category.name === 'SUPERNATURALCLASS')
    } else {
      product_items = products.data;
    }
    content = <>
